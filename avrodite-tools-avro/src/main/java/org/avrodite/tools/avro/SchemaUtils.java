  ...
  public static Map<String, Schema> createSchemaRegistry(BeanManager beanManager) {
    ...
    beanManager.beans().forEach(beanInfo -> {
      SchemaBuilder.RecordBuilder<Schema> recordBuilder = schemaBuilder
        .record(schemaName(beanInfo.getTargetRaw(), beanInfo.getTarget()))
        .prop(AvroStandard.JAVA_TYPE_SCHEMA_PROP, beanInfo.getSignature());
      SchemaBuilder.FieldAssembler<Schema> fieldAssembler = recordBuilder.fields();
      beanInfo.getFields()
        .forEach( fieldInfo ->
          fieldAssembler.name(fieldInfo.getName())
            .type(defineSchema(schemaBuilder, fieldInfo, beanManager))
            .noDefault()
        );
      registry.put(beanInfo.getSignature(), fieldAssembler.endRecord());
      log.debug(registry.get(beanInfo.getSignature()).toString(true));
    });
    return unmodifiableMap(registry);
  }

  private static String schemaName(Class<?> rawType, Type type){
    return String.join("__", rawType.getName(), hashSHA256(typeToString(type)).substring(0, 8));
  }

  static Schema defineSchema(SchemaBuilder.TypeBuilder<Schema> base, FieldInfo fieldInfo, BeanManager beanManager){
    ...
    return defineSchema(base, fingerprints, fieldInfo, 0, beanManager);
  }

  static Schema defineSchema(SchemaBuilder.TypeBuilder<Schema> base, List<TypeFingerprint> fingerprints, FieldInfo fieldInfo, int offset, BeanManager beanManager) {
    SchemaBuilder.BaseTypeBuilder<Schema> schemaBuilder = (offset == 0 && fieldInfo.isNullable())
                                                        ? base.nullable()
                                                        : base;
    switch (FieldType.of(fingerprints.get(offset))) {
      case ARRAY:
        return schemaBuilder.array().items(defineSchema(base, fingerprints, fieldInfo, offset + 1, beanManager));
      case MAP:
        return schemaBuilder.map().values().type(base.nullable().type(defineSchema(base, fingerprints, fieldInfo, offset + 1, beanManager)));
      case SHORT:
      case INT:
      ...
      default:
        return beanManager.valueCodecsIndex().keySet().stream()
                ...
                .findAny()
                .orElseGet(() -> schemaBuilder.type(schemaName(fieldInfo.getTargetTypeInfo().rawType(), fieldInfo.getTargetTypeInfo().type())));
    }
