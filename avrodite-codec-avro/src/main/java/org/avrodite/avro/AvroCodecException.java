...
public class AvroCodecException extends CodecException {
  ...
  public interface Api {
    ...
    @Message(value = "Unexpected union index value '%s' at field '%s' of type '%s'.")
    AvroCodecException unexpectedUnionIndexValue(int value, String field, String type);

    @LogMessage(level = Logger.Level.ERROR)
    @Message(value = "Illegal end of array, expected 0 but was '%s'.")
    AvroCodecException illegalArrayEnd(int value);

    @LogMessage(level = Logger.Level.ERROR)
    @Message(value = "Illegal end of map, expected 0 but was '%s'.")
    AvroCodecException illegalMapEnd(long value);

    @Message(value = "End of input.")
    AvroCodecException endOfInput();

    @Message(value = "Unexpected input, expected '%s' but was '%s'.")
    AvroCodecException unexpectedInput(int expected, int actual);

    @Message(value = "Unexpected number serialization.")
    AvroCodecException unexpectedNumber();

    @Message(value = "Unexpected error during encoding/decoding.")
